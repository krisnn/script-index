...
function pn() {
    function e(n) {
        n = $.extend({}, h, n);
        n.term = n.term || $("#s").val();
        if (n.searchSite) n.term = "site:" + n.V9 + " " + n.term;
        ...
        $.getJSON(r, {
            ...
            if (s.length) {
                ...
                for (var u = 0; u < s.length; u++) o.append(new t(s[u]) + "");
                if (!n.append) i.empty();
                o.append('<div class="clear"></div>').hide().appendTo(i).fadeIn("slow");
                var a = r.responseData.cursor;
                if (+a.estimatedResultCount > (n.page + 1) * n.perPage) $("<div>", {
                    id: "more"
                }).appendTo(i).click(function () {
                    e({
                        append: true,
                        page: n.page + 1
                    });
    ...
    function t(e) {
        ...
        case "GwebSearch":
            t = ['<div class="webResult">', '<div class="sinp">', '<a  class="linkwebsite" href="', e.unescapedUrl, '" target="_blank">', e.visibleUrl, "</a>", "</div>",
                '<div class="titlee">', '<h2><a href="', e.unescapedUrl, '" target="_blank">', e.title, "</a></h2>", "</div>", "</div>", '<div class="clear"></div>'
            ];
            break;
        case "GimageSearch":
            t = ['<div class="imageResult">', '<a target="_blank" href="', e.unescapedUrl, '" title="', e.titleNoFormatting, '" class="pic" style="width:', e.tbWidth, "px;height:", e.tbHeight, 'px;">', '<img class="img-items" src="', e.unescapedUrl, '" width="185px" ', '" height="210px" ', e.tbWidth, '" height="', e.tbHeight, '" /></a>', '<div class="clear"></div>',
                '<a class="webtitle" href="', e.originalContextUrl, '" target="_blank">', "</a>", "</div>"
            ];
            break;
        case "GvideoSearch":
            ...
            break;
        case "GnewsSearch":
            t = ['<div class="webResult">', '<div class="sinp">', '<a  class="linkwebsite" href="', e.unescapedUrl, '" target="_blank">', e.publisher, "</a>", "</div>", '<div class="titlee">', '<h2><a href="', e.unescapedUrl, '" target="_blank">', e.title, "</a></h2>", "</div>", "</div>", '<div class="clear"></div>'];
            break
        }
    ...
    var n = $("#searchSite,#searchWeb"),
        r = $("#submitButton"),
        i = $("article.blog-search,li#blog-pager-newer-link,li#blog-pager-older-link,.search-template"),
        s = $("#siteNameLabel"),
        o = $("#searchSite"),
        u = $("li.web"),
        a = $("#s"),
    ...
    $("ul.icons li").click(function () {
        ...
        p.stop().animate({
            ...
            marginLeft: e.width() / 2 - 4
        });
        h.type = e.attr("data-searchType");
        c.fadeOut()
    });
    s.append(" " + h.V9);
    o.click();
    ...
    n.change(function () {
        h.searchSite = this.id == "searchSite"
    });
    r.click(function () {
        i.hide(500)
    });
...
var un = $(".small_images"),
    an = $(".show_comments");
un.each(function () {
    $(this).attr("src", $(this).attr("src").replace(/\/s[0-9]+(\-c)?\//, "/s136-c/"))
});
XA.html('<p>' + dataposttitle + '</p>');
XB.html('<h1>' + dataposttitle + '</h1>');
an.click(function () {
    o.toggleClass("small")
});
pn();
...
var dn=function(){var e=this;this.b="//urang-kurai.googlecode.com/svn/trunk/x_31/UK/image/";this.s=["A.png","B.png","C.png","D.png"];this.i=this.s[this.random(this.s.length)];this.f=this.b+this.i;this.n=document.createElement("img");this.newSpeed().newPoint().display().newPoint().fly()};
